...
export default {
    ...
    init: async function () {
        ...
        db.exec("CREATE TABLE IF NOT EXISTS forumusers (name TEXT, dispname TEXT, avatar_url TEXT, pubkey TEXT);");
        ...
        pstatements["update_post"] = await db.prepare("UPDATE posts SET title = ?, author = ?, timestamp = ? WHERE id = ?;");
        pstatements["update_message"] = await db.prepare("UPDATE messages SET body = ?, timestamp = ?, vote_options = ?, vote_counts = ?, reaction_counts = ? WHERE id = ?;");

        pstatements["delete_post"] = await db.prepare("DELETE FROM posts WHERE id = ?;");
        pstatements["delete_message"] = await db.prepare("DELETE FROM messages WHERE id = ?;");

        pstatements["create_user"] = await db.prepare("INSERT INTO forumusers (name, dispname, avatar_url) VALUES (?, ?, ?);");
        pstatements["get_user"] = await db.prepare("SELECT * FROM forumusers WHERE name = ?;");
        pstatements["update_user"] = await db.prepare("UPDATE forumusers SET dispname = ?, avatar_url = ? WHERE name = ?;");
        pstatements["delete_user"] = await db.prepare("DELETE FROM forumusers WHERE name = ?;");
    },
    ...
    get_messages_on_post: async function (post_id, limit, offset) {
        return pstatements["get_messages_on_post"].all(post_id, limit, offset);
    },

    update_post: async function (id, title, author, timestamp) {
        return pstatements["update_post"].run(id, title, author, timestamp);
    },

    update_message: async function (id, body, timestamp, vote_options, vote_counts, reaction_counts) {
        return pstatements["update_message"].run(id, body, timestamp, vote_options, vote_counts, reaction_counts);
    },

    update_user: async function (name, dispname, avatar_url) {
        return pstatements["update_user"].run(name, dispname, avatar_url);
    },

    delete_post: async function (id) {
        return pstatements["delete_post"].run(id);
    },

    delete_message: async function (id) {
        return pstatements["delete_message"].run(id);
    },

    delete_user: async function (name) {
        return pstatements["delete_user"].run(name);
    }
}